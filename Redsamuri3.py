...
cl = LINETCR.LINE()
cl.login(token="EqZsQ7ae9nYjzdmsixy6.Lkg8ohUsFOz1yZrKJtfpHG.19Aae/UgHO/jhukcy/mryh4JDciaFIRrGZA04MPrjS0=")
cl.loginResult()

ki = LINETCR.LINE()
ki.login(token="EqdNRR1qmMQYevZvDdm6.MC3I4QsKSrV9tmqyKt6jHG.vltEUBxu+Tw2Y7RgoE9Cj9GDO/ASRFD7ewjWXytithQ=")
ki.loginResult()
        ...
        #-------------Fungsi Jam Update Finish-------------------#
                ...
                #print wait2
                    ...
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "||จับตาคนแอบเรียบร้อย||%s\n||By : ✰Redsamuri✰||\n\n> รายชื่อมีดังนี้<\n%s-=ผู้สนับสนุน=-\n•ป๊าเรส\n•พี่เรส\n•ลุงเรส\n•ไอ้เรส\n\nแล้วแต่จะเรียก😆😆\n[%s]" % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
                ...
                wait["Sider"] = True
                cl.sendText(msg.to,"เปิดระบบจับคนแอบ")
                
            elif "Sider off" in msg.text:
                if msg.to in cctv['point']:
                    cctv['cyduk'][msg.to]=False
                    wait["Sider"] = False
                    cl.sendText(msg.to, "ปิดระบบจับคนแอบ")
                else:
                    cl.sendText(msg.to, "ปิดอยู่ ดูที่การตั้งค่าคับ")
#-----------------------------------------------

#-----------------------------------------------
         #----------------Fungsi Join Group Start-----------------------#
            elif msg.text in ["เข้ามา","Join all"]:
    ...
    #-------------Fungsi Leave Group Start---------------#
            elif msg.text in ["Red bye","red bye","@bye"]:
              #if msg.from_ in admin:
                if msg.toType == 2:
                  cl.getGroup(msg.to)
    ...
    #-------------------------------------------------#
                ...
                cl.sendText(msg.to,"รับทราบ บอท Like Status Owner")
                try:
                  likePost()
                except:
                  pass
              ...
              #if msg.from_ in admin:
                print "[Command]Like executed"
                cl.sendText(msg.to,"รับทราบ บอท Like Status Teman Boss")
                try:
                  autolike()
                except:
                  pass
...
#---------------------
        if op.type == 17:
          ...
          cl.sendText(op.param1, "ยินดีต้อนรับสู่กลุ่ม  " + ">>>" + str(ginfo.name) + "<<<" + "\n" + "นะคร๊าฟ.. " + str(ginfo.name) + " :\n" + ginfo.creator.displayName + "\n\n" + "เข้ามาแล้วดูบนโน๊ตด้วยนะ !!!  😊\nอย่าลืมปิดเสียงแจ้งเตือนล่ะ 😘")
    ...
    for zx in range(0,200):
      ...
      if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
        try:
          ...
          ki.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉Auto Like by ⭐⭐Koplaxs⭐⭐👈\n\n™By ✰Ŕèďśámúŕi✰")
          ki2.like(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],likeType=1001)
          ki2.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉Auto Like by ⭐⭐Koplaxs⭐⭐👈\n\n™By ✰Ŕèďśámúŕi✰")
          print "Like"
        except:
          pass
      else:
